...
$stmtFetch = $db->prepare("SELECT * FROM blogs WHERE id = ?");
$stmtFetch->execute([$id]);
$blog = $stmtFetch->fetch(PDO::FETCH_ASSOC);
...
if (isset($_POST['kaydet'])) {
    try {
        ...
        $image_path = $blog['image_url']; // varsayılan eski fotoğraf yolu
        ...
        // Benzersiz slug denetimi (kendi id'si hariç)
        $checkStmt = $db->prepare("SELECT COUNT(*) FROM blogs WHERE slug = ? AND id != ?");
        ...
        // Güncelleme İşlemi
        $stmtUpdate = $db->prepare("UPDATE blogs SET title = ?, slug = ?, content = ?, seo_title = ?, seo_description = ?, image_url = ? WHERE id = ?");
        $stmtUpdate->execute([$title, $slug, $content, $seo_title, $seo_description, $image_path, $id]);
...
<head>
    ...
    <title>Blog Yazısı Düzenle - Yönetim Paneli</title>
    <!-- Google Fonts: Inter -->
...
<div class="main-content">
    <div class="header-section">
        ...
        <p>Mevcut blog yazısının içeriğini, resmini veya URL adresini güncelleyin.</p>
    </div>
    ...
    <div class="form-card">
        <form action="" method="POST" enctype="multipart/form-data">
            ...
            <input type="text" id="title" name="title" required value="<?= htmlspecialchars($blog['title']) ?>" placeholder="Sitede görünecek ana başlık">
            
            <label for="slug">Makale URL Adresi (Slug / Kısa Adı)</label>
            <input type="text" id="slug" name="slug" required value="<?= htmlspecialchars($blog['slug']) ?>" placeholder="Otomatik oluşturulur, dilerseniz değiştirebilirsiniz">
            
            <label for="blog_image">Kapak Fotoğrafı (Değiştirmek istemiyorsanız boş bırakın)</label>
            <?php if(!empty($blog['image_url'])): ?>
                <div style="margin-bottom: 15px;">
                    <img src="../<?= $blog['image_url'] ?>" alt="" style="max-width: 150px; border-radius: 8px; box-shadow: 0 4px 10px rgba(0,0,0,0.05);">
                </div>
            <?php endif; ?>
            <input type="file" id="blog_image" name="blog_image" accept="image/*">
            
            <label for="content">Makale İçeriği</label>
            <textarea id="content" name="content" required placeholder="Tüm makale metnini buraya giriniz..."><?= htmlspecialchars($blog['content']) ?></textarea>
            ...
            <div class="seo-box">
                ...
                <input type="text" id="seo_title" name="seo_title" required value="<?= htmlspecialchars($blog['seo_title']) ?>" placeholder="Örn: Konya Tüp Bebek Tedavi Süreçleri | Op. Dr. Necati Özçimen">
                
                <label for="seo_description">Arama Özeti (Meta Description)</label>
                <textarea id="seo_description" name="seo_description" required placeholder="Arama sonuçlarında başlığın altında görüntülenecek 160 karakterlik özet metin..." style="height: 80px;"><?= htmlspecialchars($blog['seo_description']) ?></textarea>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const hasExistingImage = <?php echo !empty($blog['image_url']) ? 'true' : 'false'; ?>;
    
    form.addEventListener('submit', function(e) {
        if (!hasExistingImage && imageInput.files.length === 0) {
            if (!confirm("Blogu fotosuz güncelliyorum emin misiniz?")) {
                e.preventDefault();
            }
        }
    });
    ...
    let userEditedSlug = true; // düzenleme modunda varsayılan olarak kullanıcı düzenleme modundadır
